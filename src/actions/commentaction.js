...
export const loadComments = (id) => async (dispatch, getState) => {
    ...
    */
    return API.request(`goals/comment/?goal=${id}`, 'GET', null, 'auth').then((response) => {
...
export const createComment = (data) => async (dispatch) => {
    ...
    return API.request('goals/comment/', 'POST', data, 'auth').then((response) => {
        ...
        if (status === 'error') {console.log("you have no permition to comment hear..");}
        if (status === 'success') {
...
export const DeleteComments = (id) => async (dispatch, getState) => {
    ...
    */
//    console.log(id);
    return API.request(`goals/comment/${id}`, 'DELETE', null, 'auth').then((response) => {
        return response.data;
    });
};


export const UpdateComments = (id, updatedcomment) => async (dispatch, getState) => {
    /*
    Update short term comments from a specific Comment id
    */
    return API.request(`goals/comment/${id}`, 'PUT', updatedcomment, 'auth').then((response) => {
        return response.data;
    });
};
...
//    console.log(commentvote);
    return API.request(`goals/comment-vote/`, 'POST', commentvote, 'auth').then((response) => {
        return response.data;
